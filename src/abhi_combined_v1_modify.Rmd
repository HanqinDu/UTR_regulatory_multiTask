...
#Replace ref_motifs_k_dr by Gasch's data --- for test
```{rewrite ref_motifs_k_dr}
...
dr_data_g$hlife <- as.numeric(as.character(dr_data_g$hlife))

ref_motifs_k_dr <- left_join(ref_motifs, dr_data_g, by = "geneName") %>%
  filter(is.finite(hlife))

ref_motifs_k_dr <- na.omit(ref_motifs_k_dr)
...
#Replace by Gasch's data alternative
```{remove log2}

##Replace by Gasch's data alternative   --- for testing
model_data_k_dr <- ref_motifs_k_dr %>%
  #convert motif frequencies to presence and then to factor
    mutate_at(motifs_raw, funs(replace(., .>= 1, 1)))%>%
    mutate_at(motifs_raw, funs(factor(.) ))%>%
    #also convert to hlife to log2fold
    mutate(log2_hlife = hlife)%>%
    select(-hlife)
...
#visualize MSE vs log(lambda plot) for CV glmnet
plot(cv_glm_k_dr, sub = "DATA: Gasch 2000 ")
