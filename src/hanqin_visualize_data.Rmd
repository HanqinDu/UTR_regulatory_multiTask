...
There are four motifs show a much higher correlation coefficient rather than the others, which, indicate the linear relationship between these motifs and expression level. One point worth mentioning is that all these motifs are also focused by Abhi where `TGTATAWT` is explained to be positively associated with RNA stability and the rest three of them are explained to be negatively associated with the RNA stability. However, Abhi is not quite confident with the conclusion of `UGUAHMNUA` as the negative relationship can only be derived from Chan's data but not Sun's data. Another point is, two of the motifs - `TGTAAATA` and `TGTATAWT` are very similar to each other though there is a large difference between their correlation coefficient.

`TGTAAATA` is from Cheng's study: `Cis-regulatory elements explain most of the mRNA stability variation across genes in yeast`

`TGTATAWT`, motif name `M24`, is from Shalgi's study: `A catalog of stability-associated sequence elements in 3' UTRs of yeast mRNAs` 
...
>Unlike the response to a 25°C to 37°C heat shock, which elicited massive and transient changes in ESR expression, the transition from 37°C to 25°C resulted in a simple rapid transition to the gene expression program characteristic of steady-state growth at 25°C, with essentially no transient features.
...
Only part of the environment changes initiate ESR. heat-shock from 25°C to 37°C is one of them. Gene expression levels under a non-ESR-initiated environmental change may probably be influenced by different motifs. Maybe it's worth to look into another group of environmental conditions?
...
A group of steady state are selected:
