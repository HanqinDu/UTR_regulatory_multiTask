...
# recent progress (2020/02/12 ~ 2020/02/18)
This week, we focused on dealing with missing value by removing genes with a high value-missing ratio and fill the rest missing value by imputation.
[(1)](#filterGene) 5.3% of genes with a high value-missing ratio (>10%) are removed from the design matrix. Since these missing values could be caused by low abundance of mRNA in the cell, there could be relatively large errors on the measurements.
[(2)](#imputation) kNN imputation has been applied to fill the rest missing value. The performance is estimated by randomly knocking out 0.1% of non-NA values, filling by imputation and computing the mean square error. [(3)](#replaceZero)In a similar way, the performance of filling missing values with 0 is computed for comparison.

We have been looking for a method to evaluate group-lasso models. generically, the usual constructs like p-values, confidence intervals, etc., do not exist for lasso estimate. In a similar study called 'Supervised group Lasso with applications to microarray data analysis' by Shuangge Ma et al., the models are evaluated by Leave-One-Out(LOO) cross-validation. However, considering that we are interested in the reasonable and reliable coefficients given to each CRE but not high accuracy models, LOO is not quite suitable for our project. On the other hand, Richard Lockhart et al.(2014) proposed a significance test called `covariance test statistic`. According to their article: "such significance test is for lasso model that does not employ resampling or data splitting, but instead uses the full data set as given, and proposes a test statistic that has a simple and exact asymptotic null distribution." 

I'm currently reading the paper `A SIGNIFICANCE TEST FOR THE LASSO` of Richard Lockhart et al.(2014) to see if the `covariance test statistic` would work in our project.
...
paste("percentage of non-zero coefficient =", count_nonzero_coefficient/count_all_coefficient)
```
...
### <a id="regulatoryEffectDecrease"> regulatory effect of motifs decrease as time pass in heat shock </a>
