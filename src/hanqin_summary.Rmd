...
```{r echo=FALSE}
eval_KNN_validation = TRUE
eval_crossValidation = TRUE
```


# recent progress (2020/03/18 ~ 2020/03/24)
To evaluate the reliability of the regression coefficient of significant motifs, we fit a linear model with selected motifs and investigate their regression coefficient and standard error. According to previous figures, we apply the gene expression levels 15 or 20 mins after specific environmental stress when the regulatory effect of motifs is the strongest. We fitted 10 models in total and plot their coefficient with error bars represent 1 standard error.[[plot](#significantCoefficient)]



# previous progress (2020/03/11 ~ 2020/03/17)
In order to select significant motifs, we calculate the weighted L2 for each motif over all the conditions so that we could consider no only the regression coefficient but also the transcript frequency at the same time:[[plot](#weightedL2)]
$$
\text{c: condition}\\
\text{g: gene}\\[2ex]
e_{mg}=
...
```{r echo=FALSE}
temp_summary_coefficient = summary_coefficient[summary_coefficient$condition == "all",]
order_motifs = temp_summary_coefficient[order(-temp_summary_coefficient$weightedsquareL2),]$motif
...
ggplot(data=temp_summary_coefficient[temp_summary_coefficient$motif %in% head(order_motifs,31),], 
  ...
  theme(axis.text.x = element_text(size = 16, angle = 70, hjust = 1),
        ...
        title = element_text(size = 14)) + 
  scale_x_discrete(name ="Motif") + 
  ggtitle("weighted L2 of the non-zero regression coefficient under all condition")
```
...
##### sorbitol
```{r echo=FALSE, fig.height=6, fig.width=8}
fullTable_Gasch_impute_knn$temp = fullTable_Gasch_impute_knn$`1M_sorbitol_15min`
formula_model = as.formula(paste("temp ~",paste(names_motifs_significant, collapse = "+")))
...
##### nitrogen
```{r echo=FALSE, fig.height=6, fig.width=8}
formula_model = as.formula(paste("Nitrogen_Depletion_30min ~",paste(names_motifs_significant, collapse = "+")))
...
##### dianide
```{r echo=FALSE, fig.height=6, fig.width=8}
fullTable_Gasch_impute_knn$temp = fullTable_Gasch_impute_knn$'1.5mM_diamide_20min'
formula_model = as.formula(paste("temp ~",paste(names_motifs_significant, collapse = "+")))
...
##### dithiothriethol
```{r echo=FALSE, fig.height=6, fig.width=8}
fullTable_Gasch_impute_knn$temp = fullTable_Gasch_impute_knn$`2.5mM_DTT_015min_dtt-1`
formula_model = as.formula(paste("temp ~",paste(names_motifs_significant, collapse = "+")))
...
##### menadione
```{r echo=FALSE, fig.height=6, fig.width=8}
fullTable_Gasch_impute_knn$temp = fullTable_Gasch_impute_knn$`1mM_Menadione_20min_redo`
formula_model = as.formula(paste("temp ~",paste(names_motifs_significant, collapse = "+")))
...
##### H2O2
```{r echo=FALSE, fig.height=6, fig.width=8}
fullTable_Gasch_impute_knn$temp = fullTable_Gasch_impute_knn$`0.32mM_H2O2_20min_redo`
formula_model = as.formula(paste("temp ~",paste(names_motifs_significant, collapse = "+")))
...
ggplot(summary_coefficient[summary_coefficient$motif %in% names_motifs_significant[1:3],], aes(x=motif, y=Estimate, group=condition, color=condition)) + 
  ...
  geom_errorbar(aes(ymin=Estimate-Std..Error, ymax=Estimate+Std..Error),width=.8,size=1,position=position_dodge(0.6))+
  theme(axis.text.x = element_text(size = 16),
        axis.text.y = element_text(size = 16),
        legend.text = element_text(size = 16),
        axis.title = element_text(size = 16)) + 
