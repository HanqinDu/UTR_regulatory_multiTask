...
ggplot(data=summary_coefficient_temp, aes(x=reorder(motif,-L2), y=, fill=condition)) +
  geom_bar(stat="identity", position=position_dodge()) +
  theme(axis.text.x = element_text(size = 16, angle = 70, hjust = 1),
        axis.text.y = element_text(size = 16),
        legend.text = element_text(size = 16),
...
```{r} 
models_all_minLambda$a0/colMeans(fullTable_Gasch[names_environmental_stress], na.rm = TRUE)
```
